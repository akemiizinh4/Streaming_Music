...
public class StreamingMusica {
    ...
    private static void gerarPlaylistAutomatica() {
        ...
        PlaylistAutomatica auto;

        if (op == 1) {
            auto = GeradorRecomendacoes.gerarTopMaisTocadas(bancoDeMusicas);
        } else if (op == 3) {
            auto = GeradorRecomendacoes.gerarAdicionadasRecentemente(bancoDeMusicas);
        } else {
            auto = new PlaylistAutomatica("Recomendadas para Você", "Recomendadas para Você");
            for (Musica m : bancoDeMusicas) {
                auto.adicionarMusica(m);
            }
        }

        System.out.println("🤖 Gerando playlist \"" + auto.getNome() + "\"...");
    ...
    private static void fazerUpgrade() {
        ...
        novoUsuario.getHistoricoReproducao().addAll(usuarioLogado.getHistoricoReproducao());
    ...
    private static void popularDadosTeste() {
        ...
        bancoDeMusicas.add(new Musica("Strobe", "deadmau5", 354, "Eletrônica"));
    }
}
